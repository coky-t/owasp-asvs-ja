...
* **暗号署名トークン** (Cryptographically Signed Token) - これらのトークンには一般的にトークン構造内にエンコードされたクレームやアサーションを含み、署名やメッセージ認証コード (MAC) などの暗号技術を使用して、このデータの真正性と完全性を検証します。最も一般的な例としては SAML アサーションと JWT があります。
...
* **ステートレスセッションメカニズム** (Stateless Session Mechanism) - ステートレスセッションメカニズムはクライアントに渡される暗号署名トークンを使用します。このトークンには、トークンを受け取って検証するサービス内には必ずしも保存されるわけではないセッション情報を含みます。実際には、必要なセキュリティコントロールを実施できるようにするために、サービスは一部のセッション情報 (JWT 失効リストなど) にアクセスする必要があります。
