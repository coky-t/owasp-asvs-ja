...
| # | 説明 | L1 | L2 | L3 | CWE |
| :---: | :--- | :---: | :---: | :---: | :---: |
| **50.2.1** | [3.4.1 から移動] Cookie ベースのセッショントークンには 'Secure' 属性が設定されている。 | ✓ | ✓ | ✓ | 614 |
| **50.2.2** | [修正, 3.4.2 から移動] Cookie ベースのセッショントークンはクライアントサイドのスクリプトで読み取れない。セッショントークン Cookie には 'HttpOnly' 属性が設定されている必要があり、セッショントークン値は Set-Cookie ヘッダフィールドを介してのみクライアントに転送される必要がある。 | ✓ | ✓ | ✓ | 1004 |
| **50.2.3** | [3.4.3 から移動] Cookie ベースのセッショントークンには、クロスサイトリクエストフォージェリ攻撃への露出を制限するために、'SameSite' 属性を利用している。 | ✓ | ✓ | ✓ | 1275 |
| **50.2.4** | [3.4.4 から移動] Cookie ベースのセッショントークンには "__Host-" プレフィックスを使用しており、Cookie は最初に Cookie を設定したホストにのみ送信されている。 | ✓ | ✓ | ✓ | 16 |
