...
| **50.6.2** | [追加, 5.3.3 から分割] コンテンツをテキストとして安全にレンダリングする関数 (createTextNode, textContent など) は、信頼できないコンテンツを HTML ページまたは DOM に適用するために使用される。コンテンツを HTML としてレンダリングするプロパティや関数 (innerHTML, document.write など) は使用してはいけない。 | ✓ | ✓ | ✓ | |
