...
| # | 説明 | L1 | L2 | L3 | CWE |
| :---: | :--- | :---: | :---: | :---: | :---: |
| **50.2.1** | [修正, 14.4.3 から移動] HTML, DOM, CSS, JSON, JavaScript インジェクション脆弱性などの XSS 攻撃に対する影響を軽減するのに役立つコンテンツセキュリティポリシー (Content Security Policy, CSP) レスポンスヘッダが設定されている。 | ✓ | ✓ | ✓ | 1021 |
...
| **50.2.5** | [14.4.7 から移動] Web アプリケーションのコンテンツはデフォルトでサードパーティのサイトに埋め込むことができない、および適切な Content-Security-Policy: frame-ancestors と X-Frame-Options レスポンスヘッダを使用して必要な場所でのみ正規のリソースの埋め込みが許可されている。 | ✓ | ✓ | ✓ | 1021 |
...
| **50.4.2** | [追加] クロスサイトスクリプトインクルージョン (XSSI) 攻撃を避けるために、機密情報は JavaScript ファイルには含まれていない。 | ✓ | ✓ | ✓ | |
...
| **50.5.3** | [追加, 14.4.2 を廃止] たとえば API や他のリソースが直接ロードされる場合に、ブラウザが不正なコンテキストで HTTP レスポンスのコンテンツや機能をレンダリングすることを防ぐために、セキュリティ制御が行われている (たとえば、ヘッダが正しいコンテキストであること (Content-Security-Policy: sandbox, Content-Disposition: attachment など) を示さない限りコンテンツを提供しない) 。 | ✓ | ✓ | ✓ | |
