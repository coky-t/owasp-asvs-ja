...
| # | 説明 | L1 | L2 | L3 | CWE |
| :---: | :--- | :---: | :---: | :---: | :---: |
| **1.3.1** | [追加] ユーザのセッション非アクティブ期間と再認証前の最大セッション有効期間が文書化され、他のコントロールと組み合わせて適切であり、その文書には NIST SP 800-63B 再認証要件からの逸脱の正当性が含まれている。 | ✓ | ✓ | ✓ | |
...
| **1.8.2** | [修正, 8.1.9 へ分割] すべての保護レベルには文書化された一連の保護案件がある。これには一般的な暗号化、完全性検証、リテンション、データのログ記録方法、ログ内の機密データに関するアクセス制御、データベースレベルの暗号化、プライバシー、使用されるプライバシー強化技法に関連する要件、およびその他の機密性要件が含まれている (ただし、これらに限定されない) 。 | | ✓ | ✓ | |
...
| **1.14.6** | [50.7.2 へ移動] | | | | |
| **1.14.7** | [追加] アプリケーションはレート制限やログ記録などの機密性の高い機能を提供するためにユーザーの真の IP アドレスを識別および利用している。 | | ✓ | ✓ | 348 |
