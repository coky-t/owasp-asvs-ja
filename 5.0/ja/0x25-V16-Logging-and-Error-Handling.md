...
| # | 説明 | レベル | #v5.0.be |
| :---: | :--- | :---: | :---: |
| **16.2.1** | 各ログエントリにはイベントが発生した時のタイムラインの詳細な調査を可能にするために必要なメタデータが含まれている。 | 2 | v5.0.be-7.1.4 |
| **16.2.2** | 時刻ソースが正しい時刻とタイムゾーンに同期している。インシデント後のフォレンジック解析を支援するためにシステムがグローバルである場合には、UTC でのみログを記録することを強く検討する。 | 2 | v5.0.be-7.1.5 |
...
| **16.4.3** | 分析、検出、警告、エスカレーションのために、ログができればリモートシステムにセキュアに転送されている。 | 2 | v5.0.be-7.3.5 |
