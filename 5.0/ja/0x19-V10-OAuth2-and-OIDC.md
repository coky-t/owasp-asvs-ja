...
| # | 説明 | レベル | #v5.0.be |
| :---: | :--- | :---: | :---: |
| **10.2.1** | コードフローが使用される場合、OAuth クライアントはコード交換のための証明鍵 (PKCE) 機能を使用するか、認可リクエストで送信された state パラメータをチェックすることで、トークンリクエストをトリガーする一般にクロスサイトリクエストフォージェリ (CSRF) として知られるブラウザベースのリクエストフォージェリ攻撃から保護している。 | 2 | v5.0.be-51.2.2 |
