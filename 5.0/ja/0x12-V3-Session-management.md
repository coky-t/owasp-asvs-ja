...
| **3.3.3** | [修正, レベル L2 > L1] 認証要素が正常に変更または削除 (リセットやリカバリーによるパスワード変更や、存在する場合、MFA 設定の更新を含む) された後、アプリケーションが他のすべてのアクティブセッションを終了するためのオプションを提供している。これはアプリケーション、Federated ログイン (存在する場合) 、Relying Party すべてに有効である必要がある。 | ✓ | ✓ | ✓ | 613 | |
...
| **3.4.2** | [修正] Cookie ベースのセッショントークンはクライアントサイドのスクリプトで読み取れない。セッショントークン Cookie には 'HttpOnly' 属性が設定されている必要があり、Cookie 値は Set-Cookie ヘッダを介してのみクライアントに転送される必要がある。 ([C6](https://owasp.org/www-project-proactive-controls/#div-numbering)) | ✓ | ✓ | ✓ | 1004 | 7.1.1 |
...
| **3.5.6** | [追加] ステートレストークンのその他のセキュリティセンシティブな属性が検証されている。例えば、JWT ではこれには issuer, subject, audience がある。 | ✓ | ✓ | ✓ | 287 | |
