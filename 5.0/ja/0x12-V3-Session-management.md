...
Cookie はセッション識別子を保護するための一般的なメカニズムであり、Cookie の具体的なセキュリティ要件は「Web フロントエンドセキュリティ」の章にあります。
...
ステートフルセッションメカニズムの場合、これはバックエンドでセッションを無効にするだけで済みます。自己完結型トークンを使用している場合、これらのトークンを取り消す方法が必要になります。
