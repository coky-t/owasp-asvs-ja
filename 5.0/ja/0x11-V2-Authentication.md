...
| **2.1.2** | [修正, 2.4.6 へ分割] 64 文字以上のパスワードが許可されている。 | ✓ | ✓ | ✓ | 521 | 5.1.1.2 |
...
| **2.1.14** | [追加, 2.1.7 から分割, レベル L1 > L3] アカウント登録またはパスワード変更時に送信されるパスワードは一連の流出したパスワードと照合されている。 | | | ✓ | | 5.1.1.2 |
...
| # | 説明 | L1 | L2 | L3 | CWE | [NIST &sect;](https://pages.nist.gov/800-63-3/sp800-63b.html) |
| :---: | :--- | :---: | :---: | :---: | :---: | :---: |
| **2.4.1** | [修正, 2.4.3, 2.4.4 からマージ] ユーザパスワードは現行のガイダンスに従って安全に構成された承認済みのパスワードハッシュアルゴリズムを使用して保存される。 | | ✓ | ✓ | 916 | 5.1.1.2 |
| **2.4.2** | [削除, 不正確] | | | | | |
| **2.4.3** | [削除, 2.4.1 へマージ] | | | | | |
| **2.4.4** | [削除, 2.4.1 へマージ] | | | | | |
| **2.4.5** | [削除, 不正確] | | | | | |
| **2.4.6** | [追加, 2.1.2 から分割] アプリケーションは長すぎるパスワードの処理によって引き起こされるサービス拒否攻撃から保護されている。 | | ✓ | ✓ | | |
...
| # | 説明 | L1 | L2 | L3 | CWE | [NIST &sect;](https://pages.nist.gov/800-63-3/sp800-63b.html) |
| :---: | :--- | :---: | :---: | :---: | :---: | :---: |
| **2.10.1** | [修正, 1.2.2 からマージ] API、ミドルウェア、データレイヤなど、アプリケーションの標準的なユーザセッションメカニズムをサポートしないバックエンドアプリケーションコンポーネント間の通信は認証されている。認証は、個別のサービスアカウント、短期トークン、または証明書ベースの認証を使用すべきであり、パスワード、API キー、特権アクセスを持つ共有アカウントなどの不変なクレデンシャルを使用すべきではない。 | | ✓ | ✓ | 287 | |
