...
| **2.1.4** | [削除, 不十分な影響] | | | | |
| **2.1.5** | [文法] ユーザがパスワードを変更できる。 | ✓ | ✓ | ✓ | 620 |
| **2.1.6** | パスワード変更機能にはユーザの現在のパスワードと新しいパスワードが必要である。 | ✓ | ✓ | ✓ | 620 |
| **2.1.7** | [修正, 2.1.13 へ分割] アカウント登録またはパスワード変更時に送信されるパスワードは少なくとも上位 3000 件の利用可能な一連のパスワードと照合されている。 | ✓ | ✓ | ✓ | 521 |
| **2.1.8** | [削除, 不十分な影響] | | | | |
| **2.1.9** | 許可される文字の種類を制限するパスワード構成規則がない。大文字、小文字、数字、または特殊文字を要求すべきではありません。 | ✓ | ✓ | ✓ | 521 |
| **2.1.10** | [修正, レベル L1 > L2] ユーザのパスワードは侵害されたことが判明するか、ユーザーが変更するまで有効のままである。アプリケーションは定期的なクレデンシャルのローテーションを要求してはいけない。 | | ✓ | ✓ | |
| **2.1.11** | 「貼り付け」機能、ブラウザパスワードヘルパー、および外部のパスワードマネージャが許可されている。 | ✓ | ✓ | ✓ | 521 |
...
| **2.1.13** | [追加, 2.1.7 から分割, レベル L1 > L3] アカウント登録またはパスワード変更時に送信されるパスワードは一連の流出したパスワードと照合されている。 | | | ✓ | |
...
| :---: | :--- | :---: | :---: | :---: | :---: |
| **2.5.1** | [削除, 不正確] | | | | |
| **2.5.2** | [文法] パスワードのヒントまたは知識ベースの認証 (いわゆる「秘密の質問」) が存在しない。 | ✓ | ✓ | ✓ | 640 |
| **2.5.3** | [削除, 6.6.2 でカバー] | | | | |
| **2.5.4** | [14.1.10 へ移動] | | | | |
| **2.5.5** | [削除, 2.2.3 でカバー] | | | | |
...
| # | 説明 | L1 | L2 | L3 | CWE |
| :---: | :--- | :---: | :---: | :---: | :---: |
| **2.6.1** | ルックアップシークレットは一度のみ使用できる。 | | ✓ | ✓ | 308 |
...
| **2.6.3** | [修正] ルックアップシークレットは予測可能な値を避けるために暗号論的にセキュアな疑似乱数生成器 (Cryptographically Secure Pseudorandom Number Generator, CSPRNG) を使用して生成されている。 | | ✓ | ✓ | 310 |
...
| **2.7.2** | [修正] 帯域外認証リクエスト、コード、またはトークンは 10 分以内に期限切れになる。 | ✓ | ✓ | ✓ | 287 |
| **2.7.3** | [文法, 2.2.6 をカバー] 帯域外認証リクエスト、コード、またはトークンは元の認証リクエストに対してのみ一度だけ使用できる。 | ✓ | ✓ | ✓ | 287 |
| **2.7.4** | [文法] 使用されているセカンダリ通信チャネルは安全であり、プライマリチャネルから独立している。 | ✓ | ✓ | ✓ | 523 |
| **2.7.5** | [削除, 不十分な影響] | | | | |
| **2.7.6** | [修正] 帯域外認証で使用されるコードは暗号的に安全な乱数生成器 (CSPRNG) を使用して生成され、少なくとも 20 ビットのエントロピーを含む (通常は 4 つのランダムな英数字または 6 つのランダムな数字で十分である)。 | | ✓ | ✓ | 310 |
| **2.7.7** | [追加] コードベースの帯域外認証メカニズムはレート制限または少なくとも64ビットのエントロピーを持つコードのいすれかを使用して、ブルートフォース攻撃から保護されている。 | | ✓ | ✓ | 307 |
...
| **2.8.3** | [文法] 承認された暗号化アルゴリズムが時間ベースのワンタイムパスワードの生成、シード、および検証に使用されている。 | | ✓ | ✓ | 326 |
| **2.8.4** | [文法, 2.2.6 をカバー] 時間ベースのワンタイムパスワードは有効期間内に一度しか使用できない。 | | ✓ | ✓ | 287 |
| **2.8.5** | [削除, 不十分な影響] | | | | |
| **2.8.6** | [修正, レベル L2 > L3] 物理的な単一要素 OTP 生成器は盗難またはその他の紛失の場合に無効にできる。場所に関係なく、ログインしたセッション全体で無効化がすぐに反映される。 | | | ✓ | 613 |
| **2.8.7** | [修正, レベル L2 > L3] 生体認証メカニズムはあなたが持っているものかあなたが知っているもののいずれかと一緒に、二次的要素としてのみ使用されている。 | | | ✓ | 308 |
| **2.8.8** | [追加] 時間ベースの OTP は、信頼できない時間やクライアントが提供する時間ではなく、信頼できるサービスからの時間ソースに基づいてチェックされている。 | | | ✓ | 367 |
...
| # | 説明 | L1 | L2 | L3 | CWE |
| :---: | :--- | :---: | :---: | :---: | :---: |
| **2.9.1** | [修正, レベル L2 > L3] 認証検証子は、検証に使用される暗号鍵を、変更 (対象鍵の場合は開示) から保護されるように保存している。これには、トラステッドプラットフォームモジュール (Trusted Platform Module, TPM)、ハードウェアセキュリティモジュール (Hardware Security Module, HSM)、またはこの安全なストレージを提供できる OS サービスを使用できる。 | | | ✓ | 320 |
| **2.9.2** | [レベル L2 > L3] チャレンジナンスは少なくとも64ビットの長さがあり、統計的に一意であるか、暗号化デバイスの寿命において一意である。 | | | ✓ | 330 |
| **2.9.3** | [修正, レベル L2 > L3] 承認された暗号化アルゴリズムが暗号化鍵の生成、シード、および検証に使用されている。 | | | ✓ | 327 |
