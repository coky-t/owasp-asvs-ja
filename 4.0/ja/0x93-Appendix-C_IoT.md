...
| **C.3** | 適用可能である場合、ASLR や DEP などのメモリ保護コントロールが組込みおよび IoT オペレーティングシステムにより有効化されていることを検証します。 | ✓ | ✓ | ✓ | 4.0 |
...
| **C.8** | ファームウェアアプリがサーバー接続のデジタル署名を妥当性確認していることを検証します。 | ✓ | ✓ | ✓ | 4.0 |
...
| **C.14** | アプリケーションおよびファームウェアコンポーネントがシェルコードラッパーやスクリプトを呼び出すことによる OS コマンドインジェクションの影響を受けないこと、またはセキュリティコントロールが OS コマンドインジェクションを防止することを検証します。 | ✓ | ✓ | ✓ | 4.0 |
| **C.15** | ファームウェアアプリが信頼できるサーバーへのデジタル署名をピン留めしていることを検証します。 |  | ✓ | ✓ | 4.0 |
| **C.16** | 改竄防止や改竄検出機能があることを検証します。 |  | ✓ | ✓ | 4.0 |
| **C.17** | チップ製造元により提供される利用可能な知的財産保護技術が有効化されていることを検証します。 |  | ✓ | ✓ | 4.0 |
| **C.18** | セキュリティコントロールがファームウェアのリバースエンジニアリングを妨げていることを検証します (冗長なデバッグシンボルの削除など) 。 |  | ✓ | ✓ | 4.0 |
