...
| :---: | :--- | :---: | :---:| :---: | :---: | :---: |
| **2.1.1** | [修正] ユーザが設定するパスワードの長さは (複数の空白が結合された後) 少なくとも 8 文字である。 ([C6](https://owasp.org/www-project-proactive-controls/#div-numbering)) | ✓ | ✓ | ✓ | 521 | 5.1.1.2 |
...
| **2.1.7** | [修正] アカウント登録およびパスワード変更時に送信されるパスワードがローカル (システムのパスワードポリシーに一致する上位 50,000 または 100,000 の最も一般的なパスワードなど) または外部 API を使用した一連の流出済みパスワードと照合している。API を使用する場合には、パスワードの流出状況の検証に平文パスワードが送信または使用されないように、ゼロ知識証明またはその他のメカニズムを使用する必要がある。サードパーティによる重大なパスワード侵害が発生した場合は、影響を受けたユーザをロックし、ユーザが新しい未流出パスワードを設定できるようにパスワード変更を強制する。 ([C6](https://owasp.org/www-project-proactive-controls/#div-numbering)) | ✓ | ✓ | ✓ | 521 | 5.1.1.2 |
...
| # | 説明 | L1 | L2 | L3 | CWE | [NIST &sect;](https://pages.nist.gov/800-63-3/sp800-63b.html) |
| :---: | :--- | :---: | :---:| :---: | :---: | :---: |
| **2.5.1** | [修正] システムが生成した初期アクティベーションまたはリカバリーシークレットがユーザに送信されている場合、それはシングルユースであり、時間制限があり、ランダムである。 ([C6](https://owasp.org/www-project-proactive-controls/#div-numbering)) | ✓ | ✓ | ✓ | 640 | 5.1.1.2 |
...
| **2.5.7** | [修正] OTP または多要素認証要素が失われた場合、登録時と同じレベルで同一性証明の証拠が実行される。 | ✓ | ✓ | ✓ | 308 | 6.1.2.3 |
...
| **2.8.8** | [追加] 時間ベースの多要素 OTP トークンの生成がクライアントのマシンではなくサーバーのシステム時間に基づいている。 | | o | ✓ | 367 | 5.1.4.2 / 5.1.5.2 |
...
| **2.9.2** | [L2>L3] チャレンジナンスは少なくとも64ビットの長さがあり、統計的に一意であるか、暗号化デバイスの寿命において一意である。 | | | ✓ | 330 | 5.1.7.2 |
