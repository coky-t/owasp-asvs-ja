...
| **2.1.10** | 定期的なクレデンシャル変更やパスワード履歴に関する要件がない。 | ✓ | ✓ | ✓ | 263 | 5.1.1.2 |
...
| **2.4.4** | bcrypt が使用されている場合、ワークファクターは検証サーバのパフォーマンスが許す限り大きくすべきであり、通常は少なくとも 10 である。 ([C6](https://owasp.org/www-project-proactive-controls/#div-numbering)) | | ✓ | ✓ | 916 | 5.1.1.2 |
| **2.4.5** | 秘密であり検証子のみが知っているソルト値を使用して、鍵生成関数の追加のイテレーションが実行される。承認された乱数ビット生成器 [SP 800-90Ar1] を使用してソルト値を生成し、少なくとも SP 800-131A の最新リビジョンで指定されている最小セキュリティ強度を提供する。秘密のソルト値はハッシュされたパスワードとは別に (例えば、ハードウェアセキュリティモジュールのような専用デバイスに) 保存すべきである。 | | ✓ | ✓ | 916 | 5.1.1.2 |
...
| # | 説明 | L1 | L2 | L3 | CWE | [NIST &sect;](https://pages.nist.gov/800-63-3/sp800-63b.html) |
| :---: | :--- | :---: | :---:| :---: | :---: | :---: |
| **2.5.1** | システムが生成した初期アクティベーションまたはリカバリーシークレットがクリアテキストでユーザーに送信されていない ([C6](https://owasp.org/www-project-proactive-controls/#div-numbering)) | ✓ | ✓ | ✓ | 640 | 5.1.1.2 |
