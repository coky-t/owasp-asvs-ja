...
| **2.2.2** | [修正] 弱い認証子 (VoIP や電子メールを使用した帯域外認証など) が認証方式として使用されていない。制限された認証子 (電話または SMS を使用して OTP を配信するために PSTN を使用するもの) はより強力な代替方式が提供される場合か、サービスがそのセキュリティリスクに関する情報をユーザーに提供する場合にのみ提供される。 | ✓ | ✓ | ✓ | 304 | 5.2.10 |
