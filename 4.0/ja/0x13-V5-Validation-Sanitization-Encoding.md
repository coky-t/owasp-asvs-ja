...
| **5.5.3** | [修正] 信頼できないウライアントとの通信では逆シリアル化が使用されていない。それができない場合は、逆シリアル化攻撃を防ぐために、オブジェクトタイプのホワイトリストのみを許可するか、クライアントが逆シリアル化先のオブジェクトタイプを定義できないようにするなど、逆シリアル化が安全に実行されるようにする。 | ✓ | ✓ | ✓ | 502 |
